...
void            ft_draw_wall(parsing_t *p_val, texture_t text, int x, int *i)
{
    ...
    if ((p_val->dda.side == NORTH || p_val->dda.side == SOUTH) && p_val->dda.rayDirX > 0)
        texx = (double)text.w - texx - 1;
    else if ((p_val->dda.side == EAST || p_val->dda.side == WEST) && p_val->dda.rayDirY < 0)
        texx = (double)text.w - texx - 1;
    ...
    *i = p_val->dda.drawstart;
    while (*i < p_val->dda.drawend)
    {
        ...
        texpos += step;
        color = mlx_get_color_value(p_val->mlx_val.mlx_ptr, (int)(text.img[text.h * texy + texx]));
