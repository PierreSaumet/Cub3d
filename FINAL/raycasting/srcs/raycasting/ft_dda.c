...
void            ft_calc_perp_draw(parsing_t *pars_val, double *z_buff)
{
    ...
    *z_buff = pars_val->dda.perpWallDist;
    pars_val->dda.lineHeight = (int)(pars_val->screenH / pars_val->dda.perpWallDist);
    pars_val->dda.drawstart = -pars_val->dda.lineHeight / 2 + pars_val->screenH / 2;
    if (pars_val->dda.drawstart < 0)
        pars_val->dda.drawstart = 0;
    pars_val->dda.drawend = pars_val->dda.lineHeight / 2 + pars_val->screenH / 2;
    if (pars_val->dda.drawend >= pars_val->screenH)
        pars_val->dda.drawend = pars_val->screenH - 1;
    if (pars_val->dda.side == NORTH || pars_val->dda.side == SOUTH )
        pars_val->dda.wallx = pars_val->posY + pars_val->dda.perpWallDist * pars_val->dda.rayDirY;
    if (pars_val->dda.side == EAST || pars_val->dda.side == WEST)
        pars_val->dda.wallx = pars_val->posX + pars_val->dda.perpWallDist * pars_val->dda.rayDirX;
...
void            ft_calc_stepx_y(parsing_t *pars_val)
{
    if (pars_val->dda.rayDirX < 0)
    {
        ...
        pars_val->dda.sideDistX = (pars_val->posX - pars_val->dda.mapX) * pars_val->dda.deltaDistX;
    }
    else
    {
        pars_val->dda.stepX = 1;
        pars_val->dda.sideDistX = (pars_val->dda.mapX + 1.0 - pars_val->posX) * pars_val->dda.deltaDistX;
    }
    if (pars_val->dda.rayDirY < 0)
    {
        pars_val->dda.stepY = -1;
        pars_val->dda.sideDistY = (pars_val->posY - pars_val->dda.mapY) * pars_val->dda.deltaDistY;
    }
    else
    {
        pars_val->dda.stepY = 1;
        pars_val->dda.sideDistY = (pars_val->dda.mapY + 1.0 - pars_val->posY) * pars_val->dda.deltaDistY;
    }
}
...
void            ft_init_raycast(parsing_t *pars_val, int x)
{
    ...
    pars_val->dda.rayDirX = pars_val->dirX + pars_val->planeX * pars_val->dda.cameraX;
    pars_val->dda.rayDirY = pars_val->dirY + pars_val->planeY * pars_val->dda.cameraX;
